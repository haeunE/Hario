...
overcome = Blueprint('overcome', __name__, template_folder='templates/overcome', static_folder='static')
# Blueprint는 클래스로 'react'는 블루프린트 이름 __name__은 현재의 모듈 이름 그리고 차례로 이 블루프린트에서 사용할
# 템플릿과 정적파일을 지정한 것

@overcome.route("/") # URL 경로를 특정 함수에 연결하는 작업을 함
def index(): # 함수    
    return render_template("overcome.html") # index.html를 보여주게 함
